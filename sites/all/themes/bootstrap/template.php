...
function get_user_full_name() {
  ...
  $name = !empty($profile) ? $profile['user']->field_fullname['und'][0]['value'] : null;
  return $name;
}

function bootstrap_image($variables) {
  ...
  return '<img' . drupal_attributes($attributes) . ' class="img-polaroid" />';
}